...
#  unique() %>% 
#  pivot_wider(names_from = c(analyte, isotype), values_from = c(data, dataIn, serostatus, serostatus_cat))
...
dataInput <- import("../15 Generate Dataframe ML/output/dataInputAll.csv") %>% 
  ...
  mutate(childhoodImmuAge = case_when(Age_group %in% c("< 30", "< 40") ~ "No",
...
plotAcuteStrat <-
  ...
  select(panel_strat, isotype, panel_detail, sampleID_metadata, analyte, dataIn, childhoodImmuAge) %>% 
  pivot_wider(names_from = analyte, values_from = dataIn, values_fn = mean) %>% 
  select(panel_strat, isotype, panel_detail, sampleID_metadata, childhoodImmuAge,
         A27L, 
         A29L,
         L1R, 
         M1R, 
  ...
  theme_bw() +
  facet_grid(isotype ~ panel_detail, labeller = as_labeller(c("1" = "Pre", "2" = "MVA",
                                                              "3" = "Mpox",
                                                              "Pre" = "Pre",
                                                              "MVA" = "MVA", "Mpox" = "Mpox"))) +
...
plotStratifiedChildhood <-
  ggarrange(plotAcuteStrat, plotSpoxStrat, nrow = 2, align = "hv", labels = "auto",
            common.legend = TRUE)

ggsave("output/plotStratified.png", plotStratifiedChildhood, width = 8, height = 12,
       dpi = 600)
...
#  filter(panel_strat %in% c("Acute", "Pre_New")) %>% 
  ...
  mutate(panel_detail = as.factor(panel_detail), 
         ...
         childhoodImmuAge = factor(childhoodImmuAge,
                                   levels = c("Yes", "No", "Ambiguous"), ordered = TRUE),
         ratio_A29L_A27L = A29L/A27L,
         ratio_M1R_L1R = M1R/L1R,
         ratio_E8L_D8L = E8L/D8L,
         ratio_A35R_A33R = A35R/A33R,
         ratio_B6R_B5R = B6R/B5R) %>% 
  select(panel_strat, isotype, panel_detail, sampleID_metadata, childhoodImmuAge,
         starts_with("ratio")) %>% 
...
####
# Statistical analysis using lmer
