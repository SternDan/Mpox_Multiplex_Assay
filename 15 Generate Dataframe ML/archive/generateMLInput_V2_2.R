...
# dataInput[complete.cases(dataInput)]


## 
# Count numbers in final dataframe
##
# Select only patient samples and select necessary columns
dataInputRawFin <-
  dataInput 
...
unique(dataInput$POX_VACC_STATUS_1)
...
unique(dataInputMultiClassComplete$sampleID_metadata)
names(dataInputMultiClassComplete)
...
# Frequency table for multiclass input
dataInputMultiClassComplete %>% 
     select(sampleID_metadata, POX_VACC, MPX_vac_final_comb, MPX_diagnose_final) %>% 
     unique() %>% 
     select(-sampleID_metadata) %>% 
     ftable()

# Frequecy table for Spox input
dataInput %>% 
  filter(panel == "SPox") %>% 
  select(sampleID_metadata,  MPX_vac_final, MPX_diagnose_final) %>% 
  unique() %>% 
  select(-sampleID_metadata) %>% 
  ftable()
...
doubleInf <-
dataInput %>% 
  filter(MPX_diagnose_final == "Yes" & MPX_vac_final == "Yes")


doubleInfMulti <-
dataInputMulticlassed %>% 
  filter(MPX_vac_final_comb == "Yes" & MPX_diagnose_final == "Yes")
...
unique(dataInputMPXV$sampleID_metadata)
dataInput %>% 
  filter(panel == "MPXV") %>% 
...
dataInput %>% 
  mutate(sampleID_core = str_remove(sampleID_metadata, "-001-[0-9][0-9]"),
         ...
         sampleID_core = str_remove(sampleID_core, "^0+"),
         sampleID_core = str_remove(sampleID_core, "-0")) %>% 
  filter(panel %in% c("MPXV")) %>% 
  group_by(panel_detail) %>% 
  select(sampleID_core) %>% 
  unique() %>% 
  count()


min(dataInputMPXVmeta$date_sampleIn)

max(dataInputMPXVdays$days_firstsample)
median(dataInputMPXVdays$days_firstsample)
mean(dataInputMPXVdays$days_firstsample)
dataInputMPXVdays %>% 
ggplot(mapping=aes(x = days_firstsample)) +
  geom_histogram()

dataInput %>% 
  filter(panel == "MVA") %>% 
  select(sampleID_metadata, panel, serostatus_factor) %>%
  unique() %>% 
  count(serostatus_factor)


dataInput %>% 
  filter(panel == "MVA") %>% 
  mutate(sampleID_core = str_remove(sampleID_metadata, ".{3}$")) %>% 
  select(sampleID_core, panel, serostatus_factor) %>%
  unique() %>% 
  count(sampleID_core)
...
dataInput %>% 
  filter(panel == "SPox") %>% 
  select(sampleID_metadata, panel_detail) %>%
  unique() %>% 
  count(panel_detail)
