...
dataFile1 <- c("input/SPox_01_Platte_1.xlsx")
...
inputData <- read_multiplex(dataFile1, dataFile2, metaWord)
...
dataInputQCIgG <-
  ...
  filter(analyte == "ahIgG", dilution == 100, 
         isotype == "IgG") %>% 
  group_by(experiment, plate_assay, sampleID_assay, isotype, dilution) %>% 
  summarise(lowIg = if_else(data < 10000, T, F),
            positiveIg = data)
...
dataInputQCIgM <-
  ...
  filter(analyte == "ahIgM", dilution == 100,
         isotype == "IgM") %>% 
  group_by(experiment, plate_assay, sampleID_assay, isotype, dilution) %>% 
  summarise(lowIg = if_else(data < 6000, T, F),
            positiveIg = data)
...
dataInput <-
  inputData %>%
  left_join(dataInputQC, by = c("experiment", "plate_assay","sampleID_assay", "isotype",
                                "dilution")) %>%
  left_join(dataInputQCHSA, by = c("experiment", "plate_assay","sampleID_assay", "isotype",
                                   "dilution"))
##
...
quantInputIgG_plate_1 <-
  dataInput%>% 
  rename(dilution_assay = dilution) %>% 
...
dataInputQuantCat <-
  ...
  left_join(threshold, by = c("isotype" = "assay", "analyte" = "antigene")) %>% 
...
dataInput <-
  dataInputQuantCat %>% 
  left_join(dataInputQuantCatDelta, by = c("experiment", "plate_assay", "isotype", "sampleID_meta"), suffix = c("", ".delta")) %>% 
  unique()
#
...
dataInputLDAAll <- dataInput %>% 
  ...
  dplyr::select(experiment, plate_assay, isotype, sampleID_meta,
...
dataOutput <-
 ...
 # left_join(LDAAll, by = c("experiment", "sampleID_meta", "plate_assay")) %>% 
  ...
  select(-GLM_value) %>% 
  left_join(ATI_infected, by = "sampleID_meta") %>% 
