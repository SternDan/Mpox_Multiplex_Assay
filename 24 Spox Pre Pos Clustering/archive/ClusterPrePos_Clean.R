...
plotPredictYoungCombined <-
  ggarrange(plotAntigensYoungPre, plotAntigensYoungMVA, align = "hv",
            ncol = 2, labels = "auto")

ggsave(filename = "output/plotPredictSeropos.png", width = 10, height = 6,
       dpi = 600)
...
predict_young_MVA_positive <-
  ...
  rowwise() %>% 
  mutate(sum_positive_ATI = sum(`ATI-N` == "positive"),
         sum_positive = sum(D8L == "positive", 
                            E8L == "positive",
                            A33R == "positive",
                            A35R == "positive",
                            B5R == "positive",
...
young_pre_export <-
  predict_young_pre_positive %>% 
  filter(sum_positive > 0) %>% 
  mutate(sum_positive_ATI = NA)

young_MVA_export <-
  predict_young_MVA_positive %>% 
  filter(sum_positive_ATI == 1)

young_positive_export <-
  rbind(young_pre_export, young_MVA_export)

export(young_positive_export, file = "output/Pre_MVA_Positive_SPox.xlsx")
sampleIDsExcluds <- data.frame(excludeIDs = sort(unique(young_positive_export$sampleID_metadata)))

export(sampleIDsExcluds, file = "output/Samples_Pre_MVA_Positive_SPox.csv")
