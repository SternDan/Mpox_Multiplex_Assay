...
# - Both IgG and IgM detection is compared
# - Final corrections on the figure
# - Export figure in publishable format in addition to png (eps)
##
...
####
# Correct analyte names in dataInput
dataInput <-
  dataInput %>% 
...
# Calculate correlation matrix and matrix of p-values (package rstatix)
pmatrixMultiIgM <- cor_pmat(dataInputCorrMultiplexIgM, method = "pearson")
matrixMultiIgM <- cor_mat(dataInputCorrMultiplexIgM, method = "pearson")
...
plotpaBaIgG <-
  ...
  scale_y_continuous(name = "Multiplex quant.", limits = c(0,5)) +
  labs(subtitle = "IgG") +
  theme_bw() +
...
##
# Generate novel Figure 1 with all data
# Plot comparison with ELISA data
Fig1_ELISA <-
  dataInputComparison %>% 
  filter(isotype != "IgA" & analyte != "Delta"  & analyte != "VACV") %>% 
  ggplot(aes(x = log10(dataIn_ELISA), y = log10(dataIn)))+ 
  geom_point()+
  scale_x_continuous(name = "ELISA quant.", limits = c(0,5)) +
  scale_y_continuous(name = "Multiplex quant.") +
  theme_bw() +
  geom_smooth(method = "lm", aes(color = isotype)) +
  scale_color_manual(name = "Isotype", values = colorblind_pal()(8)[c(2,3)]) +
...
Fig1_Multiplex <-
  ...
  geom_point()+
  scale_x_continuous(name = "Multiplex Delta quant.", limits = c(0,5)) +
  scale_y_continuous(name = "Multiplex quant.") +
  theme_bw() +
  geom_smooth(method = "lm", aes(color = isotype)) +
  scale_color_manual(name = "Isotype", values = colorblind_pal()(8)[c(2,3)]) +
...
plotCorrelationNTDeltaCol <-
  plotCorrelationNTDelta +
  scale_fill_manual(values = c(colorblind_pal()(8)[2]))

plotCorrelationNTDeltaIgMCol <-
  plotCorrelationNTDeltaIgM +
  scale_fill_manual(values = c(colorblind_pal()(8)[3]))
...
Fig1_plotDelta <-
  ggarrange(plotpaBaIgG, plotpaBaIgM ,
            ...
            plotCorrelationNTDeltaCol, 
            plotCorrelationNTDeltaIgMCol, ncol = 6, align = "hv")
...
Fig1_IFA_IgG <-
  ...
  theme_bw()+
  scale_fill_manual(name = "Isotype", values = colorblind_pal()(8)[c(2,3)]) + 
  facet_grid(isotype ~ analyte) +
  xlab("IFA (Titer)")+
  scale_y_continuous(name = "Multiplex quant.") +
  theme(plot.subtitle=element_text(hjust=0.5),
...
Fig1_combined <- ggarrange(Fig1_plotDelta,
                           Fig1_Multiplex,
                           #  Fig1_ELISA,
                           Fig1_IFA_IgG,
                           Fig1_NT_IgG,
                           nrow = 4, 
                           align = "hv",
                           heights = c(1.3, 1.5, 1, 1),
                           labels = "auto", common.legend = TRUE)
ggsave("output/Fig1_combined.png", Fig1_combined, width = 12, height = 10)
ggsave("output/Fig1_combined.eps", Fig1_combined, width = 12, height = 10)

##
# Generate supporting figure S2 with IgM results for comparison with IFA and
# comparison with NT
SFig2_IFA_IgM <-
  dataInputQuant %>% 
  left_join(metadata_IFA, by = c("sampleID_metadata", "isotype", "panel")) %>% 
  ...
  geom_point(position = position_dodge(width = 0.75), alpha = 1, aes(group = isotype), color = "black") +
  theme_bw()+
  scale_fill_manual(name = "Isotype", values = colorblind_pal()(8)[c(3)]) + 
  facet_wrap("analyte") +
  xlab("IFA (Titer)")+
  scale_y_continuous(name = "Multiplex quant.") +
  theme(plot.subtitle=element_text(hjust=0.5),
...
SFig3_combined <- ggarrange(SFig2_IFA_IgM,
                            SFig2_NT_IgM,
                            ncol = 2, 
                            align = "hv",
                            labels = "auto", common.legend = TRUE)
ggsave("output/SFig3_combined.png", SFig3_combined, width = 14, height = 8)
ggsave("output/SFig2_ELISA.png", Fig1_ELISA, width = 12, height = 4)
...
# Calculate correlation matrix and matrix of p-values (package rstatix)
pmatrixnegIFAIgM <- cor_pmat(dataInputCorIFAIgM, method = "spearman")
matrixnegIFAIgM <- cor_mat(dataInputCorIFAIgM, method = "spearman")
...
ggsave("output/Fig_S4_ROC_IFA.png",plotROCIFAcombined , width = 8, height = 10)
...
sensSpec <-
  ...
  mutate(antigene = trimws(antigene),
         antigene = case_when(antigene == "CombinedIgG" ~ "Combined",
                              TRUE ~ antigene)) %>% 
  mutate(antigene_class = case_when(antigene %in% c("A27L", "A29L", "D8L", "E8L",
                                                    "H3L", "L1R", "M1R") ~ "IMV",
                                    antigene %in% c("A33R", "A35R", "B5R", "B6R") ~ "EEV",
...
ggsave("output/Fig_S5_methComp.png", plotParameters, width = 10, height = 8)
...
###
# Linear regresseion ELISA IFA-Titer
#linRegIFA_ELISA <-
dataInputQuant %>% 
  left_join(metadata_IFA, by = c("sampleID_metadata" = "sampleID_metadata_mut", "isotype", "panel")) %>% 
  select(assaytype, dilution_assay, isotype, analyte, dataIn, titer, remarks) %>% 
  ...
  #                                          "320", "1280", "5120", "20480"),
  #                        ordered = TRUE)) %>% 
  ggplot(mapping = aes(x = log10(dataIn), y = (log10(titer)))) +
  # geom_boxplot(notch = F, alpha = 1, outlier.shape = NA) +
  #  geom_point(position = position_dodge(width = 0.75), alpha = 1, aes(group = assaytype), color = "black") +
  geom_point() +
  theme_bw()+
...
dataLinReg <-
  dataInputQuant %>% 
  left_join(metadata_IFA, by = c("sampleID_metadata" = "sampleID_metadata_mut", "isotype", "panel")) %>% 
  select(assaytype, dilution_assay, isotype, analyte, dataIn, titer, remarks, sampleID_metadata) %>% 
  filter(dilution_assay == 100, !is.na(titer)) %>% 
  filter(titer > 20) %>% 
  filter(analyte == "Delta") %>% 
  filter(isotype == "IgG") %>% 
...
ELISA_IFA.lm <- lm((titer) ~ (dataIn), data = dataLinReg)
summary(ELISA_IFA.lm)

dataLivia <- data.frame(dataIn = c(log10(225), log10(52)), ID = c("P19", "P20"))
predict(ELISA_IFA.lm, newdata = dataLivia, interval = "confidence")