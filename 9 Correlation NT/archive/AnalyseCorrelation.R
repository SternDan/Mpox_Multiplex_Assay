...
pmatrixneg <- cor_pmat(dataInputCor)
matrixneg <- cor_mat(dataInputCor)

# Save corrplot to file -> combination in Adobe Illustrator if suggestion is accepted
file_path= "output/plotCorrelationNTVACC.pdf"
...
# Calculate correlation matrix and matrix of p-values (package rstatix)
pmatrixnegNI <- cor_pmat(dataInputCorNI)
matrixnegNI <- cor_mat(dataInputCorNI)

# Save corrplot to file -> combination in Adobe Illustrator if suggestion is accepted
file_path= "output/plotCorrelationNI.pdf"
pdf(height=10, width=10, file=file_path)
plotCorrelationNI <-
